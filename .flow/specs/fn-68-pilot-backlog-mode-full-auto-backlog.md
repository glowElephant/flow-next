...
- **Jira REST API over Atlassian MCP:** portco feedback is that the Atlassian MCP is too limited for this use; the REST API token (email + API token basic auth, or PAT) is the transport that actually works in their environments and in CI. The Jira adapter leads with the API and treats MCP as a secondary rung — the inverse of Linear's MCP-first ladder.
