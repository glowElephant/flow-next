...
- **Question channel — transport-blind, multi-tracker, zero-setup:** spec-first (an `## Open Questions` section, **always written** — the universal floor) projected to whatever tracker tracker-sync has an adapter for. Backlog mode never names a tracker; it calls the normalized adapter (`postComment`/`readStatus`/`setStatus`) and inherits coverage. Target set = **GitHub, GitLab, Jira, Linear**, each reached via tracker-sync's detect-best-available ladder, preferring tooling the company already has (no flow-next-specific provisioning): `gh` / `glab` CLI sessions, a registered Linear or Atlassian MCP, or a CI env token. When no transport is detected the run is a **no-op + spec-only questions + a one-line "enable X to mirror" note** — never a crash, never a blocker. Human answers in spec or tracker comment → tracker-sync reconciles → next tick re-triages.
...
- **R13:** Multi-tracker, zero-setup. Backlog mode's surfacing is transport-blind across **GitHub, GitLab, Jira, Linear** — it calls only tracker-sync's normalized adapter, never tracker-specific code. Each tracker resolves via the discovery-ceremony probe ladder, preferring auth the company already has (`gh`/`glab` session, registered Linear/Atlassian MCP, or CI env token) — **no flow-next-specific provisioning, OAuth app, webhook, or special config required on the company's side**. The spec-first floor guarantees the loop works with zero trackers configured; the mirror auto-lights per detected transport. GitLab and Jira adapter implementations are **dependencies on tracker-sync adapter specs**, not delivered by fn-68.
...
- **Multi-tracker via the existing transport-blind interface, not bespoke per-tracker code in pilot:** GitHub/GitLab/Jira/Linear coverage rides tracker-sync's normalized adapter — the cheapest, most maintainable path and the one the adapter interface was designed for. Zero-setup is achieved by *detecting* the company's existing auth (CLI session / MCP / CI token) rather than *requiring* provisioning, with a spec-first floor so adoption needs no tracker at all. GitLab + Jira adapters are split into their own tracker-sync specs (shared infra; serve all projections) and made dependencies, not folded into fn-68.
