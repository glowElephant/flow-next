...
- **Dependency projection (fn-64):** project `depends_on_epics` as GitLab **blocked-by** relations via native **issue links** (`glab` / REST `POST /issues/{iid}/links`, `link_type=is_blocked_by`), with the same provenance discipline the GitHub adapter uses (additive-only, never delete a relation flow can't prove it created, defer-on-collision). **`is_blocked_by`/`blocks` need a Premium/Ultimate-licensed namespace** (verified 2026-06-28): native directional links + `blocking_issues_count` work in a licensed **group** project, but return **HTTP 403 `Blocked issues not available for current license`** on a **Free** namespace *and* on a **personal-namespace** project — the license is **per-namespace**, so a personal project never inherits a group's Premium/Ultimate trial. The degrade ladder covers both **without pre-detecting the license**: try `is_blocked_by` → on the 403, fall back to **`relates_to`** (works on all tiers, HTTP 201, but **directionless** — record reduced fidelity) and/or the flow-owned `<!-- flow:deps -->` fenced body block (excluded from body-merge), mirroring `github.md` § Relation transport.
...
- **Issue grain:** one flow spec ↔ one GitLab issue (the issue IID/global-id is the durable dedupe key for `sync set-tracker-id`). Back-reference written as a `flow::<id>` label + body marker (verified 2026-06-28: the `::`-named label sticks on **Free** — scoped-label mutual-exclusion *behavior* is Premium, but unneeded for a plain back-ref), mirroring the GitHub adapter's label convention. (Body writes inherit github.md's `--body-file -` shell-quoting discipline — Markdown with backticks/`$`/newlines — since GitLab bodies are GitLab-Flavored Markdown, same family as GitHub; no ADF-style translation layer is needed.)
...
- **R6:** Dependency projection (eighth method) — `depends_on_epics` → GitLab blocked-by **issue links** (additive-only, never-delete-non-ours, defer-on-collision). **`is_blocked_by` needs a Premium/Ultimate-licensed namespace** (verified 2026-06-28: native directional links work in a licensed group; 403 `Blocked issues not available for current license` on Free *and* on personal-namespace projects — license is per-namespace): try `is_blocked_by` → on 403 degrade to `relates_to` (directionless) and/or the `<!-- flow:deps -->` fenced-body fallback, recording reduced fidelity. Mirrors github.md § Relation transport.
