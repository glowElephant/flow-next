...
- **R8:** `references/gitlab.md` authored from the GitHub template; `tracker.type: gitlab` + the new `perTracker` keys documented; Codex mirror regenerated; docs updated incl. `docs/tracker-sync.md` + flow-next.dev's **BOTH navbars** (DocsRail/`site.ts` + Starlight `astro.config.mjs`) + changelog; version bumped.
