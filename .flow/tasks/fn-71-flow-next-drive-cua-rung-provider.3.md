...
**Size:** M
**Files:** repo docs + `CHANGELOG.md` + flow-next.dev (`~/work/flow-next.dev`) + all version manifests (via `scripts/bump.sh`)
...
- **flow-next.dev** (`~/work/flow-next.dev`): `src/content/docs/skills/flow-next-drive.mdx` (CUA rung + the **install/permission instructions verbatim** — curl/PowerShell installers, multi-host MCP wiring, `cua-driver permissions grant` for Accessibility + Screen Recording, `cua-driver serve` daemon, README pointer); `src/content/docs/releases/changelog.mdx` (new entry); `FLOW_NEXT_VERSION` in `src/lib/site.ts` + `package.json`. Drive page already in BOTH navbars — **no nav surgery**.
- **Version bump:** run `scripts/bump.sh patch flow-next` (target **2.1.3** unless maintainer picks minor) — updates ALL surfaces: `plugins/flow-next/.claude-plugin/plugin.json`, `.codex-plugin/plugin.json`, `.cursor-plugin/plugin.json`, root `.claude-plugin/marketplace.json`, `.agents/plugins/marketplace.json`, both README badges (and re-runs `sync-codex.sh`). Do NOT hand-edit one manifest.
...
- [ ] flow-next.dev drive page carries the CUA rung + verbatim install/permission instructions; docs-site changelog entry added; `FLOW_NEXT_VERSION` + `package.json` bumped; `pnpm build` green; no nav surgery (rung, not a new page).
- [ ] **Version bumped via `scripts/bump.sh` (target 2.1.3, patch)** — ALL surfaces updated (`.claude-plugin/plugin.json`, `.codex-plugin/plugin.json`, **`.cursor-plugin/plugin.json`**, both `marketplace.json`, both README badges), verified consistent.
